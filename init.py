...
def disp(Seed, dx, Vmax, date, loc, grid=None):
    '''
    ...
    nsteps = 8 # 30 min interp steps # 5 time interpolation steps
    N = 8 # 30 min output times
    ndays = 7 #25
    ...
    # Vmax = 30 # min amount of volume each drifter represents, approximately
    name = Seed + '_dx' + str(dx) + '_V' + str(Vmax)

    # Initial lon/lat locations for drifters
    # Start uniform array of drifters across domain using x,y coords
    if Seed == 'A':
        llcrnrlon = -93.8; urcrnrlon = -93.3; llcrnrlat = 28; urcrnrlat = 28.5; # Subset of LaCasce SCULP 1
    elif Seed == 'B':
    ...
    return nsteps, ndays, ff, tseas, ah, av, lon0, lat0, \
            z0, zpar, do3d, doturb, grid, dostream, N, T0, U, V, name
